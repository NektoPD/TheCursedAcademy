...
namespace Items.ItemVariations
{
    ...
    public class GreatIdea : Item
    {
        ...
        {
            var dmgMultNow = Mods.GetMult(Enums.StatVariations.Damage);
            var radMultNow = Mods.GetMult(Enums.StatVariations.Radius);
            var cdMultNow  = Mods.GetMult(Enums.StatVariations.AttackSpeed);

            UpdateUiStat(
                Enums.StatVariations.Damage,
                Data.Damage * dmgMultNow,
                Data.Damage * (dmgMultNow * _damageIncreasePerLevel)
            );

            UpdateUiStat(
                Enums.StatVariations.AttackSpeed,
