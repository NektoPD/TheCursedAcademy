...
{
    public static class LocalProdLangs
    {
#if RU_YG2
        ...
        public const string compressionWarning = "В WebGL-билде найдены сжатые файлы .br или .gz, но Decompression Fallback выключен. sdk-dev-proxy не добавляет необходимые заголовки Content-Encoding. Включите Player Settings → WebGL → Publishing Settings → Decompression Fallback и пересоберите проект либо установите Compression Format = Disabled.";
        ...
        public const string compressionWarning = "The WebGL build contains compressed .br or .gz files, but Decompression Fallback is disabled. sdk-dev-proxy does not add the required Content-Encoding headers. Enable Player Settings → WebGL → Publishing Settings → Decompression Fallback and rebuild the project, or set Compression Format to Disabled.";
